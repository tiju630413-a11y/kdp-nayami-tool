...
REVIEW_PERSPECTIVES = {
    ...
    "jp_quality": (
        ...
        ["japanese_quality_prompt.md（四軸の基準・唯一の参照元）"],
        "軸1（漢字水準：ジャンル上限を超える難読漢字を開いていない）・軸2（誤用：ら抜き/さ入れ/主述のねじれ/二重敬語/慣用句の誤用/重言/表記ゆれ等）・軸3（自然さ：同長文3連続・段落頭の接続詞連発・同一語尾4連続・AI常套句・句読点や！？の過剰・AI的な二重引用符\"\"）・軸4（仕上げの精度：一文一義/指示語の多用/近接の同語反復/カタカナ語の乱用/表記統一）のいずれかで『要修正』にあたる箇所がある（該当軸を明記）",
    ),
}

# jp_quality 観点にだけ足す追加指示（三軸の判定行と、ジャンル別の漢字上限）
JP_QUALITY_EXTRA = """
## 四軸の判定（findings 表の前に必ず記載）
- 漢字水準: 合格 / 要修正
- 誤用: 合格 / 要修正
- 自然さ: 合格 / 要修正   ※句読点や！？の過剰・AI的な二重引用符 "" も含む
- 仕上げの精度: 合格 / 要修正   ※一文一義・指示語の多用・近接の同語反復・表記統一

各指摘は「該当箇所の引用・軸（漢字/誤用/自然さ/仕上げ）・問題・修正案」を含めること。
