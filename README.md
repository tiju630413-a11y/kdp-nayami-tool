...
## 発信アカウントの変更（note・X 新設時）

アカウント名・URLは本文に直書きされていない。出所は2箇所だけ:
`config/settings.yaml`（note_account / note_url / x_account）と
`books/NNN/book.json`（note_url）。新設・変更したら値を書き換えて
`python3 scripts/build_epub.py NNN` を再実行すれば巻末が差し替わる
（note_url が空なら巻末のnote導線は自動省略）。promo の note 記事・X 投稿は
公開前に該当ファイル（books/NNN/promo/）の導線行を確認する。
スマホからの一言:「noteのURLが決まった。◯◯◯に差し替えて001を再ビルドして」

## モデルが変わっても品質を保つ仕組み（仕様§11）

品質はモデルではなくゲートで担保する。判定基準は全てファイル
（観点表・チェックリスト・ガイド・閾値）にあり、どのモデルでも同じ物差しで
検査される。弱いモデルで増えるのはレビュー周回数であって、ゲートを通った
本の水準は変わらない。基準値: 基準1号本（Fable 5）は重大18→5→1の3周で収束。
retro で収束曲線を比較し、周回が恒常的に増えたら settings.yaml のモデル割当を
上位に寄せる。
